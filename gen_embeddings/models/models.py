...
class ModelBuilder():
    ...
    def build_decoder(self, arch='ppm_deepsup',
                      ...
                      weights='', use_softmax=False, use_last_layer = True):
        arch = arch.lower()
        if arch == 'c1_deepsup':
...
class PPMDeepsup(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096,
                 use_softmax=False, pool_scales=(1, 2, 3, 6), use_last_layer = True):
    ...
    def forward(self, conv_out, segSize=None):
        ...
        if self.use_softmax and self.use_last_layer:  # is True during inference
            x = nn.functional.softmax(x, dim=1)
